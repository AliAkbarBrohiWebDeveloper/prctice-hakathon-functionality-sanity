...
const Images = [
  { id: 1, src: "/F1.png", header: 'HAVIT HV-G92 Gamepad', price: "$960", delprice: "$1160", star: '⭐⭐⭐⭐ (75)' },
  { id: 2, src: "/F2.png", header: 'AK-900 Wired Keyboard', price: "$120", delprice: "$160", star: '⭐⭐⭐⭐ (88)' },
  { id: 3, src: "/F3.png", header: 'IPS LCD Gaming Monitor', price: "$370", delprice: "$400", star: '⭐⭐⭐⭐ (99)' },
  { id: 4, src: "/F4.png", header: 'S-Series Comfort Chair', price: "$375", delprice: "$400", star: '⭐⭐⭐⭐ (99)' },
];
    ...
    <Wrapper>
      <section className="mt-20 md:mt-28">
        <main>
          ...
          {/* Sale Products Grid */}
          <div className="flex flex-col md:flex-row justify-center items-center gap-x-6 gap-y-6 mt-12">
            {Images.map((item) => (
              <div key={item.id} className="bg-gray-200 text-center lg:w-[270px] md:w-[240px] w-[200px] h-auto relative group">
                <div className="lg:w-[270px] md:w-[230px] w-[200px] md:h-[240px] h-[200px] lg:h-[250px] bg-[#F5F5F5] rounded-[4px] justify-center items-center flex relative">
                  <Image src={item.src} alt="" height={200} width={200} className="hover:scale-105 duration-300 mx-auto md:mx-16" />
                  <Link href={`/products/${item.id}`}>
                    <div className="absolute bottom-0 left-0 w-full h-[30px] md:h-[40px] bg-black text-white flex items-center justify-center transition-all duration-300 transform translate-y-full opacity-0 group-hover:translate-y-0 group-hover:opacity-100">
